...
public class FriendRequestService {
    ...
    public FriendRequestService(FriendRequestRepository friendRepo,
    ...
    public void sendRequest(String fromAuthUid, String toUsername) {
        ...
        System.out.println("Looking up sender by authUid: " + fromAuthUid);//----------------THIS WAS GOOD!!!!!!!!!!!!
        ...
        User fromUser = userRepo.findByAuthUid(fromAuthUid)
        ...
        // 4. Pending duplicate?
        boolean pendingExists = friendRepo
    ...
    public List<FriendRequest> outgoingRequests(String fromAuthUid) {
        String fromUsername = userRepo.findByAuthUid(fromAuthUid)
                .orElseThrow(() -> new IllegalStateException("user-not-found"))
                .getUsername();
    ...
    public void respondToRequest(String requestId, String currentUsername, boolean accept) {
        ...
        FriendRequest req = friendRepo.findById(requestId)
        ...
        if (accept) {
            ...
            User from = userRepo.findByUsername(req.getFromUsername())
                    .orElseThrow(() -> new IllegalStateException("sender-not-found"));
            User to   = userRepo.findByUsername(currentUsername)
    ...
    public List<User> listFriends(String authUid) {
        User u = userRepo.findByAuthUid(authUid)
                .orElseThrow(() -> new IllegalStateException("user-not-found"));
